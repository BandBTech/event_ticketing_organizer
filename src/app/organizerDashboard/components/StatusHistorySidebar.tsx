...
export default function StatusHistorySidebar({ history, isLoading }: StatusHistorySidebarProps) {
	...
	const getStatusIcon = (status: string, type: string) => {
		if (type === 'sales') {
			...
			if (status === 'stopped') return <StopCircle size={16} className="text-red-500" />;
		}
		...
		return (
			<div className="rounded-xl bg-white p-6 shadow-sm space-y-4">
				...
				<p className="text-gray-500 text-sm">
					{t("event.text.noStatusHistory", "No status changes recorded for this event.")}
				</p>
			</div>
		);
	}
					...
					if (item === 'DIVIDER') {
						return (
							<div key="divider" className="relative pl-10 z-10">
								...
								>
									<CaretDown size={12} />
									{t("common.button.viewMore", "View {count} more").replace('{count}', (historyList.length - 4).toString())}
								</Button>
							</div>
						);
					}
                ...
                <div className="flex flex-col gap-1 @sm:flex-row @sm:items-center @sm:justify-between">
								...
								<div className="text-sm font-medium text-gray-700">
									{historyItem.status_type === 'approval'
										? t("event.history.statusChanged", "Status updated from {old} to {new}")
											.replace('{old}', historyItem.old_status)
											.replace('{new}', historyItem.new_status)
										: t("event.history.salesChanged", "Sales {new}")
											.replace('{new}', historyItem.new_status)
									}
								</div>
								...
								<div className="flex items-center gap-1 mt-1 text-[10px] text-gray-400">
									<User size={10} />
									<span>{historyItem.changed_by_name || t("common.text.system", "System")}</span>
								</div>
							</div>
						</div>
					);
