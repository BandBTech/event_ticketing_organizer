...
import { MapPinIcon, CalendarBlankIcon, QrCodeIcon } from "@phosphor-icons/react";
import { events } from "@/app/data/events";

export default function StaffDashboardHomePage() {
  const liveEvents = events.filter((e) => e.status === "LIVE" );
  const upcomingEvents = events.filter((e) => e.status === "UPCOMING"|| e.status === "ON SALE" );
      ...
      <section>
        ...
        <div className="flex flex-col xl:flex-row gap-6">
          {liveEvents.map((event) => (
            <div
              key={event.id}
              className="bg-white dark:bg-gray-800 shadow rounded-xl overflow-hidden flex flex-col xl:flex-row w-full"
            >
              {/* Event Image */}
              <Image
                src={event.image}
              ...
              <div className="p-4 flex flex-col justify-between flex-1">
                ...
                <div className="flex justify-between mt-3">
                  <button className="border border-gray-300 dark:border-gray-600 px-3 py-1 rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                    View Detail
                  </button>
