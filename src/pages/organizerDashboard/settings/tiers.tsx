...
import { TierCardGrid } from "@/components/organizerDashboard/settings/tiers/TierCardGrid";
  ...
  return (
    <div className="text-center py-12 bg-gray-50/50 rounded-lg border border-dashed border-gray-300">
      ...
      <p className="text-lg font-medium text-gray-900 mb-1">
        {t("tierTemplates.emptyState.title", "No tier templates yet")}
      </p>
      <p className="text-gray-500 mb-4 text-sm max-w-sm mx-auto">
        {t(
          "tierTemplates.emptyState.description",
          "Create reusable tier templates to quickly set up ticket tiers for your events.",
        )}
      </p>
...
export default function TierTemplatesPage() {
  ...
  const [currentTemplate, setCurrentTemplate] = useState<TierTemplate | null>(
    null,
  );
  const [templateToDelete, setTemplateToDelete] = useState<TierTemplate | null>(
    null,
  );
  const [selectedTemplate, setSelectedTemplate] = useState<TierTemplate | null>(
    null,
  );
  ...
  const handleDeleteClick = useCallback(
    (template: TierTemplate, e?: React.MouseEvent) => {
      e?.stopPropagation();
        ...
        <ProtectedRoute>
          <div className="space-y-6 mx-auto px-1 md:p-6 md:pt-0">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {t("tierTemplates.title", "Tier Templates")}
                </h1>
                <p className="text-gray-500">
                  {t(
                    "tierTemplates.description",
                    "Manage your reusable tier templates",
                  )}
                </p>
              </div>
              <Button
            ...
            <DeleteConfirmationDialog
              title={t(
                "tierTemplates.deleteModal.title",
                "Delete Tier Template",
              )}
              deleteWhat={templateToDelete?.template_name}
              open={!!templateToDelete}
              onOpenChange={(open: boolean) =>
                !open && setTemplateToDelete(null)
              }
