...
const SEGMENT_COLORS = ["#6366f1", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#06b6d4"];
...
export function CustomerAnalyticsTab({ startDate, endDate }: CustomerAnalyticsTabProps) {
  ...
  const primaryStats = [
    ...
    },
    {
      ...
      subValue: report?.total_customers && report?.registered_users
    ...
    },
    {
      ...
      subValue: report?.total_customers && report?.guest_purchases
  ...
  const retentionStats = [
    ...
    },
    {
      ...
      label: t("reports.customerAnalytics.returningCustomers", "Returning Customers"),
    ...
    <div className="space-y-6">
      ...
      {!isLoading && !hasAnyData && (
        <div className="glass-card-lowest rounded-2xl p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            {t("reports.customerAnalytics.noDataTitle", "No Customer Data Available")}
          </h3>
          <p className="text-gray-500 max-w-md mx-auto">
            {t("reports.customerAnalytics.noDataDescription", "Customer analytics will appear here once customers start purchasing tickets.")}
          </p>
        </div>
      )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  {isLoading ? (
                    ...
                    <p className="text-sm text-gray-500">{t("reports.customerAnalytics.averageOrderValue", "Average Order Value")}</p>
                  )}
                  {isLoading ? (
                    <Skeleton className="h-7 w-24" />
                  ) : (
                    <p className="text-lg font-semibold text-gray-900">
                      {formatCurrency(report?.average_order_value ?? 0, currency, locale)}
                    </p>
                  )}
                </div>
              </div>
      ...
      {segmentChartData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Segment Pie Chart */}
          <ChartCard
            title={t("reports.customerAnalytics.customerSegments", "Customer Segments")}
            isLoading={isLoading}
          ...
          >
            <ResponsiveContainer width="100%" height={256}>
              <PieChart>
                ...
                <Tooltip
                  formatter={(value: number) => [`${value} ${t("reports.customerAnalytics.customers", "customers")}`, t("reports.customerAnalytics.count", "Count")]}
                />
                <Legend />
              </PieChart>
            </ResponsiveContainer>
          ...
          {/* Segment Bar Chart */}
          <ChartCard
            title={t("reports.customerAnalytics.segmentSpending", "Segment Spending")}
            isLoading={isLoading}
              ...
              <BarChart data={segmentBarData}>
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    if (name === "totalSpent") {
                      return [formatCurrency(value, currency, locale), t("reports.customerAnalytics.totalSpent", "Total Spent")];
                    }
                    if (name === "averageSpent") {
                      return [formatCurrency(value, currency, locale), t("reports.customerAnalytics.avgSpent", "Avg Spent")];
                    }
                    return [value, name];
                  }}
                />
                <Bar dataKey="totalSpent" fill="#6366f1" radius={[4, 4, 0, 0]} name={t("reports.customerAnalytics.totalSpent", "Total Spent")} />
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
                ...
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="text-left py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.segment", "Segment")}
                    </th>
                    <th className="text-right py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.customers", "Customers")}
                    </th>
                    <th className="text-right py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.totalSpent", "Total Spent")}
                    </th>
                    <th className="text-right py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.avgSpent", "Avg. Spent")}
                    </th>
                    <th className="text-right py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.percentage", "% of Total")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {segments.map((seg, i) => (
                    <tr key={seg.segment_name} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-gray-900 font-medium">{seg.segment_name}</span>
                        </div>
                      </td>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="text-left py-3 px-4 text-gray-500 font-medium">
                      {t("reports.customerAnalytics.columns.customer", "Customer")}
                    </th>
                    <th className="text-right py-3 px-4 text-gray-500 font-medium">
                ...
                <tbody>
                  {topCustomers.slice(0, 5).map((customer, i) => (
                    <tr key={customer.customer_id ?? i} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
